...
# library(itsadug)
library(gratia)
source("~/Documents/GitHub/laryngeal/laryngeal_my_functions.R")
...
vot_min <- 252; vot_max <- 444
f0_min <- 252; f0_max <- 444

# collect files
all_files <- list.files("~/OneDrive - University of Toronto/Projects/Laryngeal/Seoul Data/data_analysis/gam/", pattern = "\\.csv$")

exclude_ppts = c(
)

# channels
chan <- c('Cz', 'FC3', 'FC4', 'FCz', 'Fz', 'C3', 'C4', 'CP3', 'CPz', 'CP4', 'Pz')

condition_pairs <- list(
  c("dorsal_highStan_lowDevi_devi", "dorsal_lowStan_highDevi_stan"),
  c("dorsal_lowStan_highDevi_devi", "dorsal_highStan_lowDevi_stan"),
  c("glottal_highStan_lowDevi_devi", "glottal_lowStan_highDevi_stan"),
  c("glottal_lowStan_highDevi_devi", "glottal_highStan_lowDevi_stan")
)
...
for (file in all_files) {
  ...
  # get participant
  ppt <- strsplit(file, "_gam.csv")[[1]]
  
  # exclude bad participants
  if (ppt %in% exclude_ppts) {next}
  
  # select which trad erp window to use
  if (strsplit(ppt, "_")[[1]][1]=="VOT") {
    trad_min=vot_min; trad_max=vot_max
  } else if (strsplit(ppt, "_")[[1]][1]=="F0") {
    trad_min=f0_min; trad_max=f0_max
  }
  
  # get data
  raw <- read_csv(paste0("~/OneDrive - University of Toronto/Projects/Laryngeal/Seoul Data/data_analysis/gam/", file))
  # organize data
  df <- raw %>%
    dplyr::rename(item = condition) %>%
    select(c(time, all_of(chan), participant, item, epoch)) %>%
    rowwise() %>%
    dplyr::mutate(uV = mean(c_across(all_of(chan)), na.rm = TRUE)) %>%
    ungroup() %>%
  ...
  for (cond in condition_pairs) {
    ...
    dat <- diff_all %>%
      ...
      mutate(fit = ifelse(endsWith(.level_1, "_devi"), .diff, -.diff),
             se.fit = .se) %>%
    ...
    # print(fig)
    ggsave(plot = fig, width = 8, height = 5, units = "in", dpi = 300, filename = paste0("~/OneDrive - University of Toronto/Projects/Laryngeal/Seoul Data/figures/gam/", ppt, "_", cond[1], "_minus_", cond[2], ".png"))
    
  }
}

write.table(df_gam, file = "~/OneDrive - University of Toronto/Projects/Laryngeal/Seoul Data/data_analysis/gam/df_gam.txt", sep = "\t", row.names = FALSE)
